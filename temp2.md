...
Ricordo che avevo già provato una volta a cambiare la risoluzione dello schermo, ma per qualche motivo non ero arrivato ad una soluzione. Potrebbero essere state le 11 di sera e potrei aver avuto allucinazioni: riproviamo.
...
Riavvio il PC per la quarta volta e funziona. Mi prendo qualche minuto per godermi la sensazione di aver risolto un problema. Noto che anche la schermata di login, che fino ad ora si vedeva molto in piccolo, ha una risoluzione decente. _Questi sono gli effetti collaterali che mi piacciono_, penso.

Riflettendoci, non ho davvero imparato come funzionano X, LightDM e compagnia cantante. Claude mi ha dato molte soluzioni, ma non so perché una funzioni e le altre no; non ho capito perché i file `~/.xinitrc` e `~/.xprofile` non vengano eseguiti; non ho capito come LightDM lanci X. Rimango un po' deluso, nonostante possa essere consolato dal mio MacBook Pro con Linux sopra. 

Incuriosito, il giorno seguente leggo qualche discussione su forum qua e la e vedo che in molti suggeriscono di scrivere vari comandi nei file `~/.xinitrc` o `~/.xprofile`. Anche la manpage di X lo consiglia: allora Claude non allucinava! _Forse, se Claude avesse avuto accesso al mio intero file system mi avrebbe potuto dare la soluzione esatta immediatamente. Anche se potessi eseguire un'istanza del più grande modello di Claude offline e dargli in pasto il mio sistema operativo, non vorrei che risolvesse il problema per me. Preferirei stare per giorni sui forum leggendo i problemi di altri o postando una domanda io stesso invece che avere una soluzione veloce ma che non mi lascia un insegnamento, che non mi lascia nulla di più che la soluzione stessa_, penso. 
