...
An evening after a bunch of sick-leave-days I start reading the arch wiki page on [HiDPI and X Server](https://wiki.archlinux.org/title/HiDPI#X_Server) and, after reading three not so long paragraphs, think: _that must be the issue. It's the Dots Per Inch_. HiDPI displays need 192 DPI to function properly; mine is set to 96 DPI at least according to the command `xdpyinfo | grep -B 2 resolution`, which returns:
...
There is always something good in the bad, and now I am intrigued by the `xrandr` command. I run it with no flags and the result catches me off guard: it outputs all the possible resolutions of my screen (although I have to admit that any output would have caught me off guard (I didn't know what to expect)). I have an epiphany: maybe the resolution is the problem?
...
I remember already trying to change the screen resolution but not getting to a working solution. Could have been a 23:00h hallucination as it happens with the screen addiction affecting the generation I am part of. Let's try again.
...
But now I would like this command to be run automatically at each system boot. 

Claude, the smartest AI friend of the day, tells me to add the command inside an `~/.xinitrc` file, which supposedly runs every time the OS launches [Xorg](https://www.x.org/wiki/). I follow what the robot tells me to do and reboot the system, but the resolution is still the wrong one. 

_I probably did something wrong. Claude can't be wrong_, I think. So I ask Claude and it answers me assuming _it_ was wrong and tells me to check my desktop manager LightDM because it all supposedly depends on how Xorg is launched and supposedly LightDM launches Xorg. Quickly reading online about LightDM I understand that I have to create the `~/.xprofile` file and make it executable and I also understand that LightDM does not run `~/.xinitrc`. I try creating `~/.xprofile` and nothing happens. 
...
I reboot the laptop for the fourth time and the resolution is correct. _So I am the clanker?_, I think. I stop for some minutes to reflect on the peculiar feeling of solving a problem that I created myself. I notice that also the login screen, which so far was using a small font, has an OK resolution.
